...
{
    internal class GetNearbyOrdersHandler : IQueryHandler<GetNearbyOrdersQuery, List<SaleOrderDto>>
    {
        ...
        public GetNearbyOrdersHandler(ReadDbContext context, IMapper mapper)
        {
        ...
        public async Task<List<SaleOrderDto>> Handle(GetNearbyOrdersQuery request, CancellationToken cancellationToken)
        {
            ...
            var saleOrders = await _saleOrders
                ...
                .Where(s => s.Status == SaleOrderStatus.ConfirmedBySystem).ToListAsync();

            var inBoundryOrders = (buyer.ActivityArea == ActivityArea.Whole ?
                saleOrders :
                saleOrders.Where(s => buyer.ActivityArea == TehranPolygonsAreaHelper.GetActivityAreaFromPolygons(s.Latitude, s.Longitude))).Skip(request.skip).Take(request.take);
