...
internal class GetSellerContractsHandler : IQueryHandler<GetSellerContractsQuery, PaginatedResult<SellerContractDto>>
{
    private readonly DbSet<ContractReadModel> _contracts;
    private readonly IMapper _mapper;

    public GetSellerContractsHandler(ReadDbContext context, IMapper mapper)
    {
    ...
    public async Task<PaginatedResult<SellerContractDto>> Handle(GetSellerContractsQuery query, CancellationToken cancellationToken)
    {
        ...
        var paginatedResult = await contracts.
            ToPaginatedResultAsync<ContractReadModel, SellerContractDto>(query.PageIndex, query.PageSize, query.SortBy ?? string.Empty, _mapper);
