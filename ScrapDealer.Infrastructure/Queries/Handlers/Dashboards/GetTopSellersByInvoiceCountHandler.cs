...
internal class GetTopSellersByInvoiceCountHandler : IQueryHandler<GetTopSellersByInvoiceCountQuery, IEnumerable<TopSellerDto>>
{
    private readonly DbSet<InvoiceReadModel> _invoices;

    public GetTopSellersByInvoiceCountHandler(ReadDbContext context)
    {
        _invoices = context.Invoices;
    }

    public async Task<IEnumerable<TopSellerDto>> Handle(GetTopSellersByInvoiceCountQuery query, CancellationToken cancellationToken)
    {
