...
internal class GetTopBuyersByInvoiceCountHandler : IQueryHandler<GetTopBuyersByInvoiceCountQuery, IEnumerable<TopBuyerDto>>
{
    private readonly DbSet<InvoiceReadModel> _invoices;

    public GetTopBuyersByInvoiceCountHandler(ReadDbContext context)
    {
        _invoices = context.Invoices;
    }

    public async Task<IEnumerable<TopBuyerDto>> Handle(GetTopBuyersByInvoiceCountQuery query, CancellationToken cancellationToken)
    {
